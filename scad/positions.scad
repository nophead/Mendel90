...
z_slot_inset = max((washer_diameter(screw_washer(frame_screw)) + 2),
                 ceil(NEMA_width(Z_motor)) / 2 - NEMA_holes(Z_motor)[1] + washer_diameter(M3_washer) / 2 + 1) / 2;

function z_motor_bracket_hole_offset() = ceil(NEMA_width(Z_motor)) / 2 - z_slot_inset;
...
atx_bracket_width = (frame_nuts && cnc_sheets) ? 2 * (nut_radius(screw_nut(frame_screw)) + 3 * filament_width)
                                               : washer_diameter(screw_washer(frame_screw)) + 1;
...
controller_y = base_depth / 2 - base_clearance - controller_width(controller)
               - (atx_psu(psu) ? 50 : mains_inlet_width() + base_clearance); // inset by 50 to leave room for USB plug
